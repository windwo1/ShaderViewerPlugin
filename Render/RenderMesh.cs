...
namespace MeshSetPlugin.Render
{
    ...
    public class ShaderPermutation : IDisposable
    {
        ...
        public ShaderData.PermutationPair PermutationData;
        ...
        public bool LoadShaders(Device device, bool fromResources)
        {
            ...
            if (fromResources)
            {
                resourceStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("MeshSetPlugin.Shaders." + shaderFilename + ".bin");
            ...
            using (NativeReader reader = new NativeReader(fromResources ? resourceStream : new FileStream(shaderPath, FileMode.Open, FileAccess.Read)))
            {
            ...
            return LoadShaders(device, vsBytecode, psBytecode);
        }

        /// <summary>
        /// Loads in the necessary shaders and resources
        /// </summary>
        public bool LoadShaders(Device device, byte[] vsBytecode, byte[] psBytecode)
        {
            ...
            using (var reflection = new ShaderReflection(vsBytecode))
            {
                var desc = reflection.Description;
                var inputs = new List<ShaderParameterDescription>();
                for (int i = 0; i < desc.InputParameters; i++)
                {
                    var input = reflection.GetInputParameterDescription(i);
                    if (input.SemanticName.StartsWith("SV_"))
                        continue;

                    inputs.Add(input);
                }

                if (inputs.All(p => p.SemanticName == "TEXCOORD"))
                {
                    elems.RemoveAll(e => e.SemanticName == "BINORMALSIGN");

                    var texcoords = elems.Where(e => e.SemanticName == "TEXCOORD").ToList();
                    if (texcoords.Count == 2)
                    {
                    ...
                    for (int i = 0; i < elems.Count; i++)
                    {
                        ...
                        var shader = inputs[i];
                        elems[i] = new InputElement("TEXCOORD", shader.SemanticIndex, elems[i].Format, elems[i].AlignedByteOffset, elems[i].Slot, InputClassification.PerVertexData, 0);
                    }
                }
        ...
        /// </summary>
        public void SetState(DeviceContext context, MeshRenderPath renderPath, Matrix transform)
        {
            ...
            if (renderPath != MeshRenderPath.Shadows)
            {
                ...
                context.PixelShader.SetSamplers(IsFallback ? 1 : 0, pixelSamplers.ToArray());
        ...
        /// </summary>
        public void AssignParameters(RenderCreateState2 state, ref D3D11.Buffer pixelParameters, ref List<ShaderResourceView> pixelTextures)
        {
        ...
        /// </summary>
        public void AssignParameters(RenderCreateState2 state, List<ShaderParameter> pixelParamValues, List<ShaderParameter> pixelTextureValues, ref D3D11.Buffer pixelParameters, ref List<ShaderResourceView> pixelTextures)
        {
        ...
        /// </summary>
        public void AssignParameters(RenderCreateState2 state, MeshMaterial material, ref D3D11.Buffer pixelParameters, ref List<ShaderResourceView> pixelTextures)
        {
            if (pixelParameters != null)
            {
                pixelParameters.Dispose();
            ...
            if (size > 0)
            {
                using (DataStream stream = new DataStream(size, false, true))
                {
                    foreach (ShaderParameter param in PixelParameters)
                    {
                        stream.Position = param.Offset;

                        if (param.Type == ShaderParameterType.Float4)
                        {
                        ...
                        else if (param.Type == ShaderParameterType.Bool)
                        {
                            ...
                            bool value = boolParam != null ? boolParam.Value : param.BoolValue;

                            stream.Write<int>(value ? 1 : 0);
                        }
                    }
        ...
        /// </summary>
        public void AssignFallbackParameters(RenderCreateState2 state, /*MeshRenderSection*/ dynamic section, MeshMaterial material)
        {
    ...
    public class Shader : IDisposable
    {
        ...
        public ShaderPermutation GetPermutation(uint hash)
        {
            if (!permutations.ContainsKey(hash))
                return permutations.FirstOrDefault().Value;
        ...
        public bool LoadFromData(string shaderName, ShaderData.PermutationPair permutation, MeshMaterial material)
        {
            ...
            int index = 0;
            foreach (var element in permutation.VertexShader.VertexElements)
            {
                GeometryDeclarationDesc.Element elem = new GeometryDeclarationDesc.Element();

                if (element is ShaderData.VertexElement_Old oldElement)
                {
                    string usage = ((string)oldElement.Usage).Replace("VertexElementUsage_", "");
                    string format = ((string)oldElement.Format).Replace("VertexElementFormat_", "");

                    elem.Usage = (VertexElementUsage)Enum.Parse(typeof(VertexElementUsage), usage);
                    elem.Format = (VertexElementFormat)Enum.Parse(typeof(VertexElementFormat), format);
                    elem.Offset = oldElement.Offset;
                    elem.StreamIndex = oldElement.StreamIndex;

                    geomDecl.Streams[elem.StreamIndex].Classification = VertexElementClassification.PerVertex;
                }
                else if (element is ShaderData.VertexElement_New newElement)
                {
                    string format = ((string)newElement.Format).Replace("VertexElementFormat_", "");
                    string classification = ((string)newElement.Classification).Replace("VertexElementClassification_", "");

                    elem.Format = (VertexElementFormat)Enum.Parse(typeof(VertexElementFormat), format);
                    elem.Offset = newElement.Offset;
                    elem.StreamIndex = newElement.StreamIndex;

                    geomDecl.Streams[elem.StreamIndex].Classification = (VertexElementClassification)Enum.Parse(typeof(VertexElementClassification), classification);
                }
            ...
            nameHashLookup.Add(shaderName, geomDecl.Hash);

            foreach (var param in material.TextureParameters)
            {
                string name = param.ParameterName;
                string value = App.AssetManager.GetEbxEntry(param.Value.External.FileGuid).Name;

                perm.PixelTextures.Add(new ShaderParameter(name, ShaderParameterType.Tex2d, value));
            }

            foreach (var sampler in permutation.PixelShader.SamplerStates.OrderBy(s => s.index))
            {
                var desc = sampler.desc;

                Filter filter = (Filter)desc.Filter;
                TextureAddressMode addressU = (TextureAddressMode)desc.AddressU;
                TextureAddressMode addressV = (TextureAddressMode)desc.AddressV;
                TextureAddressMode addressW = (TextureAddressMode)desc.AddressW;
                float mipLodBias = desc.MipLODBias;
                int maxAniso = (int)desc.MaxAnisotropy;
                float minLod = desc.MinLOD;
                float maxLod = desc.MaxLOD;
                Color4 borderColor = new Color4(desc.BorderColor[0], desc.BorderColor[1], desc.BorderColor[2], desc.BorderColor[3]);
                Comparison comparison = (Comparison)desc.ComparisonFunc;
                ...
                perm.PixelSamplerDescs.Add(samplerDesc);
            }

            UpdatePixelParameters(permutation.PixelShader, perm, material, shaderName);

            return true;
        }

        public void UpdatePixelParameters(ShaderData.ShaderGraphPermutation permutation, ShaderPermutation perm, MeshMaterial material, string shaderName)
        {
            perm.PixelParameters.Clear();

            int offset = 0;
            foreach (var param in permutation.ExternalValueConstants)
            {
                var type = param.Type == "ExternalValueConstantType_Vec" ? ShaderParameterType.Float4 : ShaderParameterType.Bool;

                if (type == ShaderParameterType.Float4)
                {
                    var vecParam = material.VectorParameters.Find(p => p.ParameterName == param.Name);
                    var val = vecParam != null ? vecParam.Value : param.DefaultValue;

                    var parameter = new ShaderParameter(shaderName, type, val.x, val.y, val.z, val.w);

                    if ((offset % 16) + 16 > 16)
                        offset = ((offset + 15) / 16) * 16;

                    parameter.Offset += offset;
                    perm.PixelParameters.Add(parameter);
                    offset += 16;
                }
                else
                {
                    var val = param.DefaultValue;
                    var boolParam = material.BoolParameters.Find(p => p.ParameterName == param.Name);

                    bool defaultBool = param.DefaultValue.x == 1;
                    bool boolVal = boolParam != null ? boolParam.Value : defaultBool;

                    var parameter = new ShaderParameter(shaderName, type, boolVal);
            ...
            perm.PixelConstantsSize = ((offset + 15) / 16) * 16;
        }
        ...
        public bool Load(string filename, bool fromResource)
        {
            ...
            if (fromResource)
            {
                resourceStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("MeshSetPlugin.Shaders." + filename + ".xml");
                if (resourceStream == null)
                    return false;
            }
            else
            ...
            try
            {
                ...
                xmlDoc.Load(fromResource ? resourceStream : new FileStream(path + ".xml", FileMode.Open, FileAccess.Read));
                    ...
                    while (repeat)
                    {
                        ...
                        using (NativeWriter writer = new NativeWriter(new FileStream(path + ".bin", FileMode.Create)))
                        {
                            ...
                            foreach (ShaderPermutation permutation in permutations.Values)
                            {
                                ...
                                macros.Add(new ShaderMacro("PERM_" + permutation.Name, null));
                                ...
                                File.WriteAllText(path + ".generated.hlsl", shaderCode);

                                System.Diagnostics.Debug.WriteLine(shaderCode);
        ...
        private int GetNewFormatSize(string format)
        {
            switch (format)
            {
                case "VertexElementFormat_Half2": return 4;
                case "VertexElementFormat_Half4": return 8;
                case "VertexElementFormat_Float3": return 8;
                case "VertexElementFormat_UShort4": return 8;
                case "VertexElementFormat_UByte4": return 4;
                case "VertexElementFormat_UByte4N": return 4;

                default:
                    App.Logger.LogError("Unrecognized format: " + format);
                    return 0;
            }
        }
    ...
    #region -- Meshes --
    public class RenderCreateState2
    {
    ...
    public class ShaderLibrary : IDisposable
    {
        ...
        public ShaderPermutation GetUserShader(string name, GeometryDeclarationDesc geomDecl, bool fromResource)
        {
            ...
            if (info.Shader == null)
            {
                ...
                if (!info.Shader.Load(info.XmlDescriptor, fromResource))
                    return null;
            }
    ...
    public class MeshRenderBase
    {
        ...
        public virtual void Render(DeviceContext context, MeshRenderPath renderPath, Matrix transform)
        {
        }
    }
    public class MeshRenderShape : MeshRenderBase, IDisposable
    {
        ...
        /// </summary>
        public static MeshRenderShape CreateSphere(RenderCreateState2 state, string inName, string shaderName, bool shaderIsResource, float radius, int tessellation)
        {
            ...
            return new MeshRenderShape(state, inName, shaderName, shaderIsResource, vertices, indices);
        }

        /// <summary>
        /// Constructs a renderable box
        /// </summary>
        public static MeshRenderShape CreateCube(RenderCreateState2 state, string inName, string shaderName, bool shaderIsResource, int width, int height, int depth)
        {
        ...
        /// </summary>
        public static MeshRenderShape CreatePlane(RenderCreateState2 state, string inName, string shaderName, bool shaderIsResource, int width, int depth)
        {
            List<ShapeVertex> vertices = new List<ShapeVertex>()
            {
        ...
        /// </summary>
        private MeshRenderShape(RenderCreateState2 state, string inName, string shaderName, bool shaderIsResource, List<ShapeVertex> vertices, List<ushort> indices)
        {
            ...
            permutation = state.ShaderLibrary.GetUserShader(shaderName, geomDecl, shaderIsResource);
            permutation.LoadShaders(state.Device, shaderIsResource);
        ...
        /// Renders this shape
        /// </summary>
        public override void Render(DeviceContext context, MeshRenderPath renderPath, Matrix transform)
        {
            ...
            permutation.SetState(context, renderPath, transform);
