...
using MeshMaterial = MeshSetPlugin.Render.MeshMaterial;
...
namespace MeshSetPlugin
{
    ...
    public class MeshSetPreviewSettings
    {
        ...
        [Editor(typeof(FrostySliderEditor))]
        [SliderMinMax(0, 1000.0f, 0.1f, 1.0f, true)]
        [EbxFieldMeta(EbxFieldType.Float32)]
        ...
        [EbxFieldMeta(EbxFieldType.Pointer, "TextureAsset")]
        public PointerRef SkyboxTexture { get; set; }
        [Category("Scene")]
        [DisplayName("Visual Environment")]
        [EbxFieldMeta(EbxFieldType.Pointer, "VisualEnvironmentBlueprint")]
        public PointerRef VisualEnvironment { get; set; }
        ...
        public MeshSetPreviewSettings()
        {
			...
			var skybox = App.AssetManager.GetEbxEntry(Config.Get<string>("SkyboxTexture", "", ConfigScope.Game));
            if (skybox != null)
            {
                SkyboxTexture = new PointerRef(new EbxImportReference
                {
                    FileGuid = skybox.Guid,
                    ClassGuid = ((dynamic)App.AssetManager.GetEbx(skybox).RootObject).GetInstanceGuid().ExportedGuid
                });
            }
		}
    }
    ...
    public class FrostyMeshSetEditor : FrostyAssetEditor
    {
        ...
        private void PgPreviewSettings_OnModified(object sender, ItemModifiedEventArgs e)
        {
            ...
            else if (e.Item.Name == "BloomStrength")
            {
                screen.BloomStrength = previewSettings.BloomStrength;
            }
            else if (e.Item.Name == "TimeScale")
            {
                screen.TimeScale = previewSettings.TimeScale;
            }
        ...
        private void VariationsComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (pgDetails.SelectedClass is MeshSetVariationDetails meshVar)
                screen.LoadMaterials(0, GetVariation(meshVar));
        }
        ...
        private void FrostyMeshSetEditor_Loaded(object sender, RoutedEventArgs e)
        {
            if (firstTimeLoad)
            {
                ...
                var skybox = App.AssetManager.GetEbxEntry(Config.Get<string>("SkyboxTexture", "", ConfigScope.Game));
                screen.SetSkyboxTexture(skybox);
